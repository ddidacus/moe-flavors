#!/bin/bash
# Runs one or more scripts/eval/eval_router.py variants in parallel
# (GPU-pinned). Not meant to be run directly -- invoked by
# scripts/cluv/eval_router.sh via `cluv submit <cluster> -- bash
# scripts/cluv/_eval_router_run.sh <cluster> <variant> [variant ...]`.
set -euo pipefail
CLUSTER="$1"; shift
OUT_DIR="evals/${CLUSTER}/router_$(date +%F)"
