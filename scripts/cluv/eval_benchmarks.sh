...
# parallel (GPU-pinned), mirroring scripts/eval/run_benchmarks.sh's
# pattern but targeting checkpoints/<variant>_<CLUSTER> -- the naming
# scripts/cluv/train_<variant>.sh saves to.
#
# Usage: CLUSTER=fir bash scripts/cluv/eval_benchmarks.sh [variant ...]
...
#   cluv submit <cluster> -- python scripts/eval/eval_benchmarks.py --variant merge --out-dir evals/<cluster>/<date>
set -euo pipefail
cd "$(dirname "$0")/../.."
...
cluv submit --autocommit "$CLUSTER" -- bash scripts/cluv/_eval_benchmarks_run.sh "$CLUSTER" "${VARIANTS[@]}"
echo "eval_benchmarks [${VARIANTS[*]}] -> submitted to $CLUSTER"
