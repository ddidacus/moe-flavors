#!/bin/bash
# OLMoE-7B variant of train_controller_baseline.sh --
...
# controller_baseline's 16/1 for Phi-tiny-MoE, which had the most headroom
# of the four GRPO-like variants -- smoke_test_context.sh measured only
# ~69GB/80GB peak at 2048+2048): a conservative, UNVALIDATED estimate for
# OLMoE's ~2x weight footprint -- there's no OLMoE smoke test yet. Consider
# a short dry run (small --max-samples/--num-steps) before trusting the
# full 200-step budget.
#
# Usage: CLUSTER=tamia bash scripts/cluv/train_controller_baseline_olmoe.sh
set -euo pipefail
cd "$(dirname "$0")/../.."
...
cluv submit --autocommit "$CLUSTER" --time=1-00:00:00 -- accelerate launch --multi_gpu --num_processes 4 \
    ...
    --cache-size 16 --cache-layer -1 --deliberation-cost 0.02 \
    --wandb-run-name "controller-baseline-olmoe-${CLUSTER}" --save-dir "checkpoints/controller_baseline_olmoe_${CLUSTER}"
echo "controller_baseline (OLMoE-7B) -> submitted to $CLUSTER"
