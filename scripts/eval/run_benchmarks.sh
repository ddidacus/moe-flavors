...
# processes within this single job -- same pattern as run_router.sh's
...
# evals/benchmarks/<today>; override with OUT_DIR=... if needed.
#
# Usage: sbatch scripts/eval/run_benchmarks.sh <variant1> [variant2] [variant3]
#        sbatch scripts/eval/run_benchmarks.sh merge   # CPU-only, no GPU needed
#
# Time budget: short-unkillable hard-caps at 3h, so NUM_SEEDS/LIMIT/
# MAX_GEN_TOKS are trimmed down from eval_benchmarks.py's own defaults
...
OUT_DIR="${OUT_DIR:-evals/benchmarks/$(date +%F)}"

if [ "$1" = "merge" ]; then
    python scripts/eval/eval_benchmarks.py --variant merge --out-dir "$OUT_DIR"
    exit $?
fi
...
for variant in "$@"; do
    ...
    CUDA_VISIBLE_DEVICES=$gpu TRITON_CACHE_DIR=/tmp/triton_cache_${SLURM_JOB_ID}_${variant} \
        python scripts/eval/eval_benchmarks.py --variant "$variant" --out-dir "$OUT_DIR" \
