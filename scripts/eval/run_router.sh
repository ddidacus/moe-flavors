...
# processes within this single job -- same pattern as run_benchmarks.sh.
...
# eval_router.py's docstring), which is much slower than the old
...
# eval_benchmarks.py's VARIANT_CHECKPOINTS[variant] (the mila
# run_finetune_moe_*.sh naming convention). "base" never takes a checkpoint.
#
# Usage: sbatch scripts/eval/run_router.sh cache_sft temporal_moe
#        CHECKPOINT_DIR_CACHE_SFT=checkpoints/small-scale/cache_sft_tamia \
#            sbatch scripts/eval/run_router.sh cache_sft
