...
\centering
\small
\caption{Results on Phi-tiny-MoE-instruct (1024 held-out prompts), at the trained working-set
capacity ($c=4$, top-2). The base model's row reports absolute lm-eval-harness scores; every
other row reports $\Delta$, the difference from the base model's own score on that task
(positive is better). Misses is the mean number of working-set misses per sequence (lower is
better); ECI is the normalized routing entropy $H(p)/\log(E)$ (1.0 = load-balanced, near 0 =
collapsed onto a few experts). Absolute scores, tokens/second, and the LoRA caveat above apply
equally to every row here; see Table~\ref{tab:phiresults_full} in Appendix~\ref{app:fulltables}
for the absolute values this table's deltas are computed from.}
\resizebox{\textwidth}{!}{%
...
\midrule
Base (untuned)          & 0.614 & 0.345 & 0.622 & 0.024 & 0.090 & 0.571 & 651.1 & 0.892 \\
\midrule
Causal LM alone (SFT)   & +0.004 & $-$0.032 & $-$0.047 & +0.025 & 0.000  & 0.561 & 694.9 & 0.890 \\
Working-set reward       & $-$0.004 & $-$0.002 & $-$0.044 & $-$0.006 & $-$0.017 & 0.624 & 434.3 & 0.888 \\
Working-set reward + SFT & +0.004 & +0.001 & $-$0.061 & +0.013 & $-$0.019 & \textbf{0.773} & \textbf{441.3} & 0.721 \\
Option-critic controller & $-$0.032 & $-$0.011 & $-$0.106 & $-$0.024 & $-$0.079 & \textbf{1.000} & \textbf{0.1} & 0.279 \\
MELINOE                 & $-$0.009 & $-$0.012 & $-$0.111 & $-$0.024 & $-$0.073 & 0.880 & 236.3 & 0.674 \\
\bottomrule
\end{tabular}%
}
\label{tab:phiresults}
\end{table}

\begin{table}[h]
\centering
\small
\caption{Results on OLMoE-1B-7B (1024 held-out prompts), at the trained working-set capacity
($c=16$, top-8), same convention as Table~\ref{tab:phiresults}: the base row is absolute, every
other row is $\Delta$ from the base model. The combined working-set reward + SFT objective for
this backbone was still running at submission time and is omitted. See
Table~\ref{tab:olmoeresults_full} in Appendix~\ref{app:fulltables} for absolute scores and
tokens/second.}
\resizebox{\textwidth}{!}{%
\begin{tabular}{l ccccc | ccc}
\toprule
...
Objective & MMLU & MMMLU & GSM8K & HumanEval & MATH & Hit ratio & Misses & ECI \\
\midrule
Base (untuned)          & 0.563 & 0.380 & 0.673 & 0.244 & 0.058 & 0.687 & 1869.0 & 0.895 \\
\midrule
Causal LM alone (SFT)   & $-$0.014 & $-$0.010 & $-$0.081 & $-$0.055 & $-$0.022 & 0.679 & 1930.0 & 0.897 \\
Working-set reward       & 0.000  & $-$0.013 & $-$0.009 & +0.085 & +0.003 & \textbf{0.759} & \textbf{1004.2} & 0.895 \\
Option-critic controller & $-$0.036 & $-$0.069 & $-$0.131 & $-$0.104 & $-$0.056 & 0.684 & 2012.7 & 0.888 \\
MELINOE                 & $-$0.071 & $-$0.058 & $-$0.238 & $-$0.165 & $-$0.056 & 0.763 & 1541.9 & 0.838 \\
\bottomrule
\end{tabular}%
}
\label{tab:olmoeresults}
\end{table}

On both backbones the working-set reward alone (no supervised anchor) is competitive with
or better than the base model on most tasks, and its combination with the supervised loss
(Phi-tiny-MoE only, so far) raises the hit ratio further still, to 0.773, while also
recovering the throughput cost of the reward-only variant -- both option-critic and
MELINOE lose several points of downstream accuracy relative to the base model, most
sharply on MATH, GSM8K, and HumanEval, for a comparable or, on Phi-tiny-MoE, a much larger
hit-ratio gain (option-critic reaches a hit ratio of 1.0, i.e. every expert access falls
within its working set, at the cost of collapsing HumanEval and MATH to near zero).
Tokens/second is computed against the base model's own compute pass plus each objective's
own working-set miss count (see the LoRA note above), which isolates the offload benefit
from each adapter's own inference-time cost; under this measure the working-set-reward-only
variant is the one exception that still trails the base model on both backbones despite a
higher hit ratio and fewer raw misses, which we attribute to its shorter average
completions dividing a comparable total latency over fewer generated tokens, and flag as a
limitation of a purely token-averaged throughput metric rather than evidence against the
working-set benefit itself; combining the reward with the SFT anchor (Phi-tiny-MoE)
removes this effect entirely. The option-critic controller's near-total hit ratio comes at
the cost of routing collapse -- its ECI drops to 0.28 on Phi-tiny-MoE (from 0.89 for the
base model), meaning the controller has learned to route almost every token through a
small, fixed subset of experts rather than genuinely exploiting sequence-level locality;
MELINOE's ECI (0.67) sits between the two, consistent with a smaller but real drop in
routing diversity. The working-set reward, by contrast, keeps ECI close to the base
model's (0.89 on Phi-tiny-MoE, 0.90 on OLMoE) while still cutting misses by a third to a
half, indicating its hit-ratio gains come from exploiting temporal reuse of a still
load-balanced expert population rather than from collapsing the router.
...
the comparison, bought with the largest accuracy regressions -- while the working-set
reward and its SFT-combined variant sit closer to the vertical zero-delta line while still
improving hit ratio over the base model, and OLMoE's working-set reward is the only point
in the entire comparison with a positive score delta together with a hit-ratio gain,
falling in the upper-right quadrant we would want every objective to occupy.

\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{figures/delta_score_vs_hit_ratio.png}
...
each backbone's own base model (dotted line at $\Delta=0$). Color distinguishes backbone
(Phi-tiny-MoE-instruct vs. OLMoE-1B-7B), each point labeled by objective. The dashed
staircase is the Pareto front over both axes jointly (higher hit ratio \emph{and} improved
downstream accuracy is better, toward the upper right); only OLMoE's working-set reward
combines a positive score delta with a hit-ratio gain, and the option-critic controller
sets the frontier's hit-ratio extreme at the cost of the largest downstream regression.}
...
(Section~\ref{sec:conditioning}); Table~\ref{tab:conditioned} reports OLMoE-1B-7B swept
across $c \in \{8, 16, 32\}$ (Phi-tiny-MoE's sweep over $c \in \{2, 4, 8\}$ was still
running at submission time and is omitted). As expected, both hit ratio and misses improve
monotonically with capacity, from 0.507 at $c=8$ to 0.901 at $c=32$; tokens/second also
...
trading off against the hit-ratio gain as it does for the fixed-capacity working-set
reward of Table~\ref{tab:olmoeresults}. A single prompt-conditioned policy therefore lets a deployment pick a
hit-ratio/throughput operating point at inference time without retraining, unlike the
fixed-capacity variants of Section~\ref{sec:results-unconditioned}.

\begin{table}[h]
\centering
\small
\caption{OLMoE-1B-7B prompt-conditioned policy swept over every working-set capacity it
was trained on (1024 held-out prompts per capacity, top-8). Tokens/second is normalized
against the (unconditioned) base model's own compute pass, as in Tables~1-2.}
...
8  & 0.507 & 445.6 \\
16 & 0.741 & 459.0 \\
32 & \textbf{0.901} & \textbf{476.9} \\
\bottomrule
\end{tabular}
\end{table}

For reference, the same checkpoint evaluated unconditioned on the downstream task suite
(lm-eval-harness prompts do not carry a \texttt{[CACHE\_SIZE=X]} prefix, so this is a single
run rather than swept per capacity) scores MMLU 0.544, MMMLU 0.374, GSM8K 0.662, HumanEval
0.268, and MATH 0.053 -- in the same range as the unconditioned objectives of
Table~\ref{tab:olmoeresults}, indicating the conditioning prefix does not measurably harm
...
Tables~\ref{tab:phiresults} and~\ref{tab:olmoeresults} in Section~\ref{sec:results-unconditioned}
report every objective's lm-eval-harness scores as $\Delta$ from its own backbone's base model,
to make relative comparisons across five tasks and two backbones easier to read at a glance.
Tables~\ref{tab:phiresults_full} and~\ref{tab:olmoeresults_full} below give the same results in
absolute form, together with tokens/second (omitted from the main-text tables).

\begin{table}[h]
\centering
\small
\caption{Absolute results on Phi-tiny-MoE-instruct (1024 held-out prompts), at the trained
working-set capacity ($c=4$, top-2) -- the source values for Table~\ref{tab:phiresults}'s
$\Delta$ columns. Tokens/second follows the estimated offloaded-inference latency model of
Appendix~\ref{app:stack} (compute plus a per-expert transfer penalty on every working-set
miss) rather than a wall-clock trace on real offloading hardware, and is normalized against
the base model's own compute pass (see the LoRA note in Section~\ref{sec:results}) so that
adapters touching the expert projections are not penalized twice.}
\resizebox{\textwidth}{!}{%
...
Working-set reward + SFT & 0.618 & 0.346 & 0.561 & 0.037 & 0.071 & \textbf{0.773} & \textbf{441.3} & 0.721 & 509.6 \\
Option-critic controller & 0.582 & 0.334 & 0.516 & 0.000 & 0.011 & \textbf{1.000} & \textbf{0.1} & 0.279 & 514.8 \\
...
capacity ($c=16$, top-8) -- the source values for Table~\ref{tab:olmoeresults}'s $\Delta$
columns. Tokens/second normalized against the base model's own compute pass as above. The
combined working-set reward + SFT objective for this backbone was still running at
submission time and is omitted.}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lccccccccc}
\toprule
Objective & MMLU & MMMLU & GSM8K & HumanEval & MATH & Hit ratio & Misses & ECI & Tokens/s \\
...
Working-set reward       & 0.563 & 0.367 & 0.664 & 0.329 & 0.061 & \textbf{0.759} & \textbf{1004.2} & 0.895 & 273.9 \\
...
Figures~\ref{fig:phi_base}--\ref{fig:olmoe_scaling_tps} collect every routing-trace and
...
\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{figures/phi_base_expert_trace.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{figures/phi_sft_baseline_expert_trace.png}
\caption{Phi-tiny-MoE-instruct, causal LM alone (SFT) -- expert routing trace at layers 8/16/24.}
\label{fig:phi_sft}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{figures/phi_cache_reward_expert_trace.png}
\caption{Phi-tiny-MoE-instruct, working-set reward alone -- expert routing trace at layers 8/16/24.}
\label{fig:phi_reward}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{figures/phi_cache_sft_expert_trace.png}
\caption{Phi-tiny-MoE-instruct, working-set reward + SFT (combined objective) -- expert routing trace at layers 8/16/24.}
\label{fig:phi_cachesft}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{figures/phi_controller_baseline_expert_trace.png}
\caption{Phi-tiny-MoE-instruct, option-critic controller baseline -- expert routing trace at layers 8/16/24.}
\label{fig:phi_controller}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{figures/phi_melinoe_baseline_expert_trace.png}
\caption{Phi-tiny-MoE-instruct, MELINOE baseline -- expert routing trace at layers 8/16/24.}
\label{fig:phi_melinoe}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{figures/olmoe_base_expert_trace.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{figures/olmoe_sft_baseline_expert_trace.png}
\caption{OLMoE-1B-7B, causal LM alone (SFT) -- expert routing trace at layers 4/8/12.}
\label{fig:olmoe_sft}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{figures/olmoe_cache_reward_expert_trace.png}
\caption{OLMoE-1B-7B, working-set reward alone -- expert routing trace at layers 4/8/12.}
\label{fig:olmoe_reward}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{figures/olmoe_controller_baseline_expert_trace.png}
\caption{OLMoE-1B-7B, option-critic controller baseline -- expert routing trace at layers 4/8/12.}
\label{fig:olmoe_controller}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{figures/olmoe_melinoe_baseline_expert_trace.png}
\caption{OLMoE-1B-7B, MELINOE baseline -- expert routing trace at layers 4/8/12.}
\label{fig:olmoe_melinoe}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{figures/olmoe_prompt_conditioned_scaling_hit_ratio.png}
\caption{OLMoE-1B-7B, prompt-conditioned policy -- working-set hit ratio as a function of
the conditioned capacity $c \in \{8, 16, 32\}$ (descending on the $x$-axis), corresponding
to Table~\ref{tab:conditioned}.}
\label{fig:olmoe_scaling_hit}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{figures/olmoe_prompt_conditioned_scaling_tokens_per_second.png}
\caption{OLMoE-1B-7B, prompt-conditioned policy -- estimated tokens/second (own, adapter-inflated
measurement, not base-normalized) as a function of the conditioned capacity $c \in \{8, 16, 32\}$
(descending on the $x$-axis), corresponding to Table~\ref{tab:conditioned}.}
\label{fig:olmoe_scaling_tps}
\end{figure}
